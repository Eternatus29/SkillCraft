...
const addCourse = async (res, courseId, userId) => {
  ...
  try {
    const emailResponse = await emailSender(enrolledUser.email, 'Congratulations for buying new course from SkillCraft', courseEnrollmentEmailTemplate(enrolledCourse.title, enrolledUser.firstName));

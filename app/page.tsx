...
async function getStats() {
  ...
  } catch { return { courses: 0, reviews: 0 } }
}
...
export default async function HomePage() {
  const [courses, stats, categories, user] = await Promise.all([
    getFeaturedCourses(),
    getStats(),
    getCategories(),
    getSession(),
  ...
  return (
    <>
      ...
      <main className="flex-1">
        {/* Hero */}
        <section className="relative bg-linear-to-br from-violet-950 via-violet-800 to-indigo-900 text-white overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 lg:py-36">
            <div className="max-w-3xl">
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight mb-6 tracking-tight">
                Build Skills That{' '}
                <span className="text-transparent bg-clip-text bg-linear-to-r from-violet-300 to-pink-300">
                  Open Doors
                </span>
              </h1>
              ...
              <div className="flex flex-wrap gap-4">
                <Link href="/courses" className="flex items-center gap-2 bg-white text-violet-800 font-semibold px-6 py-3 rounded-xl hover:bg-violet-50 transition-colors">
                  Explore Courses <ArrowRight className="w-4 h-4" />
                </Link>
                <Link href="/signup?role=Instructor" className="flex items-center gap-2 border border-violet-400 text-white font-semibold px-6 py-3 rounded-xl hover:bg-violet-700/30 transition-colors">
                  Teach on SkillCraft
                </Link>
              </div>
            </div>
          ...
          <div className="relative bg-violet-950/60 border-t border-violet-800/50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 grid grid-cols-3 gap-6 text-center sm:flex sm:justify-around">
              <div><p className="text-2xl font-bold text-white">{stats.courses}+</p><p className="text-xs text-violet-300 mt-0.5">Courses</p></div>
              <div><p className="text-2xl font-bold text-white">{stats.reviews}+</p><p className="text-xs text-violet-300 mt-0.5">Reviews</p></div>
              <div><p className="text-2xl font-bold text-white">4.8</p><p className="text-xs text-violet-300 mt-0.5">Avg. Rating</p></div>
            </div>
          </div>
        </section>
          ...
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-14">
            ...
            <div className="flex flex-wrap gap-3">
              {categories.map((cat: any) => (
                <Link key={cat._id} href={`/courses?category=${encodeURIComponent(cat.name)}`}
        ...
        {/* CTA */}
        <section className="bg-linear-to-r from-violet-700 to-indigo-700 text-white">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16 text-center">
            <h2 className="text-3xl font-bold mb-4">Ready to start learning?</h2>
            <p className="text-violet-200 mb-8 text-lg">Join thousands of learners already growing their skills on SkillCraft.</p>
            <Link href="/signup" className="inline-flex items-center gap-2 bg-white text-violet-700 font-semibold px-8 py-3.5 rounded-xl hover:bg-violet-50 transition-colors">
